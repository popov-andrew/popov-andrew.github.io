...
export const mandelbulbShader = glsl`
...
vec2 GetDist(vec3 p) {
    ...
    float iterCount = 0.0; // For coloring
    
    for (int i = 0; i < int(ITERATIONS); i++) {
        r = length(z);
        if (r > 2.0) break;

        // For color: track iteration
        iterCount = float(i);
        ...
        //Replace w/ size and variation variables eventually...
        float Power = 10.0 + (7.0 * sin(u_time * 0.1)); 
    ...
    // Normalize
    float orbitFactor = iterCount / ITERATIONS;
...
float GetLight(vec3 p) {
    ...
    // 1st Light
    vec3 lightPos1 = vec3(0, 11, 3);
    lightPos1.xz += vec2(sin(u_lightTime), cos(u_lightTime)) * 7.0;
    
    vec3 l1 = normalize(lightPos1 - p);
    float dif1 = clamp(dot(n, l1), 0.0, 1.0);
    
    // 1st Shadow
    float d1 = RayMarch(p + n * SURF_DIST * 2.0, l1);
    if(d1 < length(lightPos1 - p)) dif1 *= 0.1;
    
    totalDif += dif1;

    // 2nd Light
    vec3 lightPos2 = vec3(0, -8, 0);
    lightPos2.yz += vec2(sin(u_lightTime * 0.8), cos(u_lightTime * 0.8)) * 6.0;

    vec3 l2 = normalize(lightPos2 - p);
    // 0.5 intensity so it acts as a fill light
    float dif2 = clamp(dot(n, l2), 0.0, 1.0) * 0.5;

    // 2nd Shadow
    float d2 = RayMarch(p + n * SURF_DIST * 2.0, l2);
    if(d2 < length(lightPos2 - p)) dif2 *= 0.1;
...
void main() {
    ...
    vec3 ro = vec3(0, 0, -3); // Camera Setup: Ray Origin

    ro.yz *= rot(-u_viewRotation.y); // Y-axis rotation
    ro.xz *= rot(-u_viewRotation.x); // X-axis rotation

    vec3 lookAt = vec3(0, 0, 0); //(Always look at center 0,0,0)
    vec3 forward = normalize(lookAt - ro);
    vec3 right = normalize(cross(vec3(0, 1, 0), forward));
    vec3 up = cross(forward, right);
    
    vec3 rd = normalize(forward + uv.x * right + uv.y * up); //Normalize distance
    ...
    if(d < MAX_DIST) {
        ...
        // Ground Plane
        if (p.y < -1.9) {
            col = vec3(0.1, 0.1, 0.15) * GetLight(p);
        } else {
            vec3 albedo = mixOklab(u_colorA, u_colorB, t);
            float lighting = GetLight(p);
            col = albedo * lighting;
            col += albedo * 0.2; 
        }
